...
pub const WireObject = Trait(.{
    ...
    .errd = fn () void,
    .sendMessage = fn (mem.Allocator, std.Io, Message) anyerror!void,
    .server = fn () bool,
    .getId = fn () u32,
}, .{Object});

pub fn called(self: WireObject, gpa: mem.Allocator, io: std.Io, id: u32, data: []const u8, fds: []const i32) !void {
    ...
    if (methods.len <= id) {
        ...
        self.vtable.@"error"(self.ptr, arena.allocator(), io, self.vtable.getId(self.ptr), msg);
        return;
    }
