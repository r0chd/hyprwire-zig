...
pub fn Trait(comptime methods: anytype, comptime embedded: anytype) type {
    const embedded_traits = switch (@typeInfo(@TypeOf(embedded))) {
        .null => embedded,
        .@"struct" => |s| if (s.is_tuple) embedded else .{embedded},
        else => .{embedded},
    };

    const has_embeds = @TypeOf(embedded_traits) != @TypeOf(null);

    // Generate VTable type with function pointers
    const VTableType = generateVTableType(methods, embedded_traits, has_embeds);

    // Create the validation namespace
    const ValidationNamespace = CreateValidationNamespace(methods, embedded_traits, has_embeds);
    ...
    return struct {
        ...
        fn computeEmbedLayout(comptime Embed: type) EmbedInfo {
            const outer_fields = std.meta.fields(VTable);
            const embed_fields = std.meta.fields(Embed.VTable);
            if (embed_fields.len == 0) {
                @compileError("Embedded trait has no methods to project");
            }

            var start_index: ?usize = null;
            outer: for (outer_fields, 0..) |field, i| {
                if (!std.mem.eql(u8, field.name, embed_fields[0].name)) continue;
                if (i + embed_fields.len > outer_fields.len) break;

                var matches = true;
                for (embed_fields, 0..) |ef, j| {
                    if (!std.mem.eql(u8, ef.name, outer_fields[i + j].name)) {
                        matches = false;
                        break;
                    }
                }
            ...
            if (start_index == null) {
                @compileError("Embedded trait layout not found in outer VTable; ensure method names are unique and contiguous");
            }

            return .{
                .trait = Embed,
                .byte_offset = @offsetOf(VTable, outer_fields[start_index.?].name),
            };
        }

        fn buildEmbedInfos() []const EmbedInfo {
            comptime {
                if (!has_embeds) return &.{};

                const Embeds = @TypeOf(embedded_traits);
                var infos: [std.meta.fields(Embeds).len]EmbedInfo = undefined;
                for (std.meta.fields(Embeds), 0..) |embed_field, i| {
                    const embed = @field(embedded_traits, embed_field.name);
                    infos[i] = computeEmbedLayout(embed);
                }
                return &infos;
            }
        }

        fn getEmbedInfo(comptime Tgt: type) EmbedInfo {
            comptime {
                const infos = buildEmbedInfos();
                for (infos) |info| {
                    if (info.trait == Tgt) return info;
                }
                @compileError("Target trait is not embedded in this trait");
            }
        }

        pub const embed_infos = buildEmbedInfos();

        /// Project this trait value to one of its embedded traits.
        pub fn asEmbed(self: Self, comptime Tgt: type) Tgt {
            comptime {
                if (!@hasDecl(Tgt, "VTable") or !@hasDecl(Tgt, "validation")) {
                    @compileError("Tgt must be a trait generated by Trait()");
                }
            }

            const info = comptime getEmbedInfo(Tgt);
            const vtable_addr: usize = @intFromPtr(self.vtable) + info.byte_offset;
            const tgt_vtable: *const Tgt.VTable = @ptrFromInt(vtable_addr);
...
fn generateVTableType(comptime methods: anytype, comptime embedded_traits: anytype, comptime has_embeds: bool) type {
    comptime {
        ...
        // Add methods from embedded traits first
        if (has_embeds) {
            const Embeds = @TypeOf(embedded_traits);
            for (std.meta.fields(Embeds)) |embed_field| {
                const embed = @field(embedded_traits, embed_field.name);
                // Recursively get the VTable type from the embedded trait
...
fn CreateValidationNamespace(comptime methods: anytype, comptime embedded_traits: anytype, comptime has_embeds: bool) type {
    return struct {
        ...
        fn collectMethodNames() []const []const u8 {
            comptime {
                ...
                // Count methods from embedded traits
                if (has_embeds) {
                    for (std.meta.fields(Embeds)) |embed_field| {
                        const embed = @field(embedded_traits, embed_field.name);
            ...
            comptime {
                ...
                if (has_embeds) {
                    for (std.meta.fields(Embeds)) |embed_field| {
                        const embed = @field(embedded_traits, embed_field.name);
                        if (embed.validation.hasMethod(method_name)) {
                            traits[index] = @typeName(@TypeOf(embed));
                            index += 1;
                        }
                    }
                }
        ...
        /// Checks if this trait has a specific method
        ...
        pub fn incompatibilities(comptime ImplType: type) []const Incompatibility {
            comptime {
                ...
                // Check embedded traits
                if (has_embeds) {
                    for (std.meta.fields(@TypeOf(embedded_traits))) |embed_field| {
                        const embed = @field(embedded_traits, embed_field.name);
